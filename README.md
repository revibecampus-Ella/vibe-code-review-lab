...
- 공개 Repository 키워드 검색 및 적합도 점수 높은 순 정렬 (동점 시 Star 높은 순)
- 링크 모음·학습 자료처럼 진단할 코드가 없는 저장소를 감점하고 사유 표시
- 검색 결과를 왼쪽에 유지하고 Repo를 한 개씩 독립 진단
- README와 주요 설정·소스·테스트 파일 읽기
- 코드 구조, 오류 가능성, 보안 위험 신호, 테스트 상태 진단
- 배포 URL이 있을 때만 UI·UX 진단
- 확인됨·추정됨·확인 필요 구분
- 위험도 신호와 발견 항목 수를 한눈에 보여주는 종합결론
- 분야별 판단을 아코디언으로 정리하고 위험·주의 항목만 펼쳐서 제시
- 다음 행동 제시
...
- 도메인 이름을 실제로 조회해 사설 IP를 가리키면 요청 전에 차단
- 배포 URL의 리디렉션 목적지를 단계마다 다시 검증
- 화면 응답 수집을 최대 500KB로, 조회 전체를 8초로 제한
- 만료된 검색 캐시 자동 정리
