...
- 검색 결과를 왼쪽에 유지하고 Repo를 한 개씩 독립 진단
...
이 프로젝트는 공개 데이터를 읽기 전용으로 조회합니다. Fork, Commit, Push, PR 생성, 코드 수정, 패키지 설치, 빌드 실행, 자동 테스트, 데이터베이스 변경 및 운영 환경변수 조회를 수행하지 않습니다.

분석 결과는 실제로 확인한 범위만 사용하며 다음처럼 구분합니다.
...
- `raw.githubusercontent.com`은 검색이 아니라 이미 선택한 Repository 파일의 원문 조회에 사용합니다.
- 실제 빌드와 테스트를 실행하지 않으므로 테스트 통과나 운영 배포 가능 여부를 확정하지 않습니다.
- 배포 URL이 없거나 실제 화면을 읽지 못하면 UI·UX 결과를 생성하지 않습니다.
